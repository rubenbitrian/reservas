...
class UserController extends AbstractController {
  ...
  public function index(UserRepository $repo, SignUpRepository $repo2):Response {
    ...
    return $this->render(
      'user/index.html.twig', ['controller_name' => 'UserController', 'lst' => $lst, 'registro' => $registro,]
    );
  }
  ...
  public function edit($id = 0, UserRepository $repo, Request $request, UserPasswordHasherInterface $passwordHasher) {
    ...
    if ($id != 0) {
      ...
      $form = $this->createForm(
        UserType::class, $user, ['required' => FALSE]
      );

      if ($user == NULL) {
        $this->addFlash(
          'danger', 'Es necesario un usuario para poder editarlo.'
        );
    ...
    return $this->render(
      'user/perfil.html.twig', ['frmUser' => $form->createView(), 'user' => $user,]
    );
  }
